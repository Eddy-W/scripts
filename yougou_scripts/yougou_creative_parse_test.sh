...
where substr(oa.orderid,4,4)='$dd_t'
;
"
cmd="hive -e \"$sql\""
echo "$cmd"
...
sql="
 ...
 from  pb_ifc_bidlog where dd='$dd' and ifcbids_.pricebid_>0  and rawlog_.solutionid_='256'  )ca
 left outer join (
...
select $media_str_3 ,c.bidn,c.winn,c.winrate,
c.ws,c.clk,c.ctr,c.cpm,c.cpc,b.ordern,b.ordersum
,b.ordersum/b.ordern orderpervalue,b.ordersum/c.ws roi
from
(select $media_str_2 from $sidbid_media_table) a left outer join
(
...
from  pb_ifc_bidlog where dd<='$dd' and dd>='$dd_pre' and ifcbids_.pricebid_>0
