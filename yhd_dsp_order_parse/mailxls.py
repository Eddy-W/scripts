...
def send_mail(options,dt, dd, attach_file, LOG):
    subject = "[%s]yhd_dsp_performance" % dd

    body_content = u"\n\nDear all\n\n"
    body_content += (u"附件为%s一号店DSP报表, 有任何疑问请及时和我联系。\n" % dd)
    body_content += (u"此邮件为系统自动发送邮件，请勿回复，谢谢!\n")
    body_content += (u"\n\n谢谢\n王文涛\n")

    recipients = "assure_xu@allyes.com,kewei_hong@allyes.com,yuying_shi@allyes.com,shuangping_shen@allyes.com,fake_li@allyes.com,hfeng@fountontech.com,lchen@fountontech.com"
    cc_recipients = "tony_zhang@allyes.com,shawn_gao@allyes.com,hua.cao@fountontech.com,wentao_wang@allyes.com"

    #recipients = "wentao_wang@allyes.com"
    #cc_recipients = "wentao_wang@allyes.com"
...
def send_raw_mail(options, LOG, subject, body_content, recipients, cc_recipients, attach_file):
    ...
    cmd = "mail_exchange -s '%s' -f '%s' -r '%s' --cc_recipients '%s' -a '%s' -u 'wentao_wang' -p '2238681Xwww' "\
